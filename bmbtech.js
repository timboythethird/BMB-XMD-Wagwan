...
zk.ev.on("group-participants.update", async (update) => {
  ...
  for (const participant of update.participants) {
    let profilePic;
    try {
      profilePic = await zk.profilePictureUrl(participant, "image");
    } catch {
      profilePic = "https://telegra.ph/file/265c672094dfa87caea19.jpg"; // Default profile pic
    }
    ...
    if (update.action === "add") {
      ...
      await zk.sendMessage(channelJid, {
        image: { url: profilePic },
        caption: `😢 *@${username}* left or was removed from *${groupName}*.`,
        mentions: [participant]
      });
    }
  }
