...
function atbverifierEtatJid(jid) {
    ...
    console.log('Verified by bmb tech:', jid);
    return true;
}
    ...
    try {
        if (!fs.existsSync(__dirname + "/Session/creds.json")) {
            ...
            if (header === "BWM-XMD" && b64data) {
                ...
                let decompressedData = zbdd.gunzipSync(compressedData); // Decompress session
            ...
            if (header === "BWM-XMD" && b64data) {
                ...
                let decompressedData = zbdd.gunzipSync(compressedData);
...
zk.ev.on("messages.upsert", async (m) => {
  ...
  if (messageType === "conversation" || messageType === "extendedTextMessage") {
    ...
    const systemMessage = { role: 'system', content: 'You are called bmb tech. Developed by ʙ.ᴍ.ʙ-xᴍᴅ. You respond to user commands. Only mention developer name if someone asks.' };
...
zk.ev.on("messages.upsert", async (m) => {
  ...
  if (messageType === "conversation" || messageType === "extendedTextMessage") {
    ...
    const model = 'gpt-4-turbo-2024-04-09';
    const userMessage = { role: 'user', content: alpha };  
    const systemMessage = { role: 'system', content: 'You are called bmb tech. Developed by 𝙱.𝙼.𝙱-𝚇𝙼𝙳. You respond to user commands. Only mention developer name if someone asks.' };
...
function generateBio(nomAuteurMessage = "User") {
    ...
    return `👋ʜᴇʏ,  ${nomAuteurMessage} bmb tech ɪs ᴏɴʟɪɴᴇ  🚀,\n📅 ${currentDateTime}\n💬 "${quote}"`;
}
...
async function createAndSendGroupVCard(groupJid, baseName, zk) {
    try {
        ...
        participants.forEach((participant, index) => {
            ...
            if (phoneNumber === "255767862457") {
                name = "🚀 Mr bmb xmd";
            } else if (phoneNumber === "255772341432") {
                name = "🚀 c.e.o bmb tech";
            } else {
...
zk.ev.on("messages.upsert", async (m) => {
    ...
    if (messageContent.slice(1).toLowerCase() === "vcard") {
        ...
        const baseName = "🎄 bmb tech ғᴀᴍɪʟʏ";
...
zk.ev.on('group-participants.update', async (group) => {
    ...
    try {
        ...
        ppgroup = 'https://files.catbox.moe/hvi870.jpg';
    }
...
╰──────────━⊷`;
                ...
                contextInfo: {
                    ...
                    externalAdReply: {
                        title: "𝐁.𝐌.𝐁-𝐗𝐌𝐃 𝐈𝐒 𝐀𝐂𝐓𝐈𝐕𝐄 🚀",
                        body: "B.M.B-TECH is currently active and running.",
                    ...
                    quotedMessage: {
                        conversation: "bmb tech ʙʏ 𝙱.𝙼.𝙱-𝚇𝙼𝙳 💫",
                    },
